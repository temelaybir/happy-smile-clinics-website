...
async function initializePages() {
  ...
  let existingData = {};
  try {
    const fileContent = await fs.readFile(pagesFile, 'utf-8');
    existingData = JSON.parse(fileContent);
  } catch (error) {
    console.log('No existing pages.json found, creating new one');
  }
  
  // Merge with existing data (preserving any existing pages)
  const mergedData = { ...pages, ...existingData };
  
  // Write back to file
  await fs.writeFile(pagesFile, JSON.stringify(mergedData, null, 2));
  
  console.log('Pages initialized successfully!');
  console.log('Available pages:');
  Object.values(mergedData).forEach(page => {
    console.log(`- /${page.slug} - ${page.title}`);
  });
}

initializePages().catch(console.error);